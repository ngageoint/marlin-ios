...
class FogSignalImage: UIImage, @unchecked Sendable {
    ...
    class func drawArcLines(
        ...
        innerSignalPath.addArc(withCenter: center, radius: radius - (radius / 1.5),
                               startAngle: 315 * (CGFloat.pi / 180.0), endAngle: 0 * (CGFloat.pi / 180.0),
                               clockwise: true)
        innerSignalPath.lineWidth = arcWidth
        color.setStroke()
    ...
    convenience init?(
        ...
        let labelColor = UIColor.label.resolvedColor(
            with: UITraitCollection(traitsFrom: [.init(userInterfaceStyle: darkMode ? .dark : .light)]))
        ...
        let image = renderer.image { _ in
            ...
            if drawArcs {
                FogSignalImage.drawArcLines(center: center, radius: radius, color: circleColor, arcWidth: finalArcWidth)
            }
