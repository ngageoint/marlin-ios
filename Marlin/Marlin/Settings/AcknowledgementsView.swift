...
[MIT license](https://raw.githubusercontent.com/sindresorhus/ExceptionCatcher/main/license).
"""),
        Acknowledgement(
            title: "GeoJSON",
            copyright: "Copyright (c) 2020 Kilian Koeltzsch",
            license: """
This product includes software licensed under the \
[MIT license](https://github.com/kiliankoe/GeoJSON/blob/main/LICENSE).
""")
    ]
    init() {
        if let path = Bundle.main.url(
            forResource: "Settings",
            withExtension: "bundle")?.appendingPathComponent("Acknowledgements.plist"),
            let dictionary = NSDictionary(contentsOf: path) {
            for library in dictionary["PreferenceSpecifiers"] as? [Dictionary<String, Any>] ?? [] {
                if let title = library["Title"] as? String,
                   !title.isEmpty,
                   title != "Acknowledgements" {
                    acknowledgements.append(
                        Acknowledgement(title: title,
                                        copyright: library["FooterText"] as? String,
                                        license: library["License"] as? String)
                    )
                }
            }
        }
